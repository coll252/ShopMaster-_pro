...
async function sendMail(to, subject, html) {
  if (!emailUser || !emailPass) {
    console.log("Email skipped: missing email credentials.");
    return;
  }

  await mailer.sendMail({
    from: `"ShopMaster Pro" <${emailUser}>`,
    to,
    subject,
    html,
  });
...
async function connectDatabase() {
  if (!process.env.DB_HOST || !process.env.DB_USER || !process.env.DB_PASSWORD || !process.env.DB_NAME) {
    throw new Error("Missing DB_HOST, DB_USER, DB_PASSWORD, or DB_NAME environment variable.");
  }
...
async function createTables() {
  ...
  await db.query(`
    CREATE TABLE IF NOT EXISTS products (
      id INT AUTO_INCREMENT PRIMARY KEY,
      name VARCHAR(160) NOT NULL DEFAULT 'Product',
  ...
  await db.query(`
    CREATE TABLE IF NOT EXISTS customers (
      id INT AUTO_INCREMENT PRIMARY KEY,
      name VARCHAR(160) NOT NULL DEFAULT 'Customer',
      email VARCHAR(180),
      phone VARCHAR(60),
      avatar TEXT,
      address TEXT,
  ...
  await db.query(`
    CREATE TABLE IF NOT EXISTS traffic_sources (
      id INT AUTO_INCREMENT PRIMARY KEY,
      source_name VARCHAR(100) NOT NULL DEFAULT 'Direct',
  ...
  await db.query(`
    CREATE TABLE IF NOT EXISTS sales_reports (
      id INT AUTO_INCREMENT PRIMARY KEY,
      month_name VARCHAR(20) NOT NULL DEFAULT 'Jan',
  ...
  await db.query(`
    CREATE TABLE IF NOT EXISTS chats (
      ...
      customer_id INT NULL,
      message TEXT,
      sender VARCHAR(40) DEFAULT 'customer',
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    )
  `);
  ...
  await db.query(`
    CREATE TABLE IF NOT EXISTS emails (
      id INT AUTO_INCREMENT PRIMARY KEY,
      recipient VARCHAR(180) NOT NULL DEFAULT 'unknown@email.com',
  ...
  await db.query(`
    CREATE TABLE IF NOT EXISTS mpesa_payments (
      id INT AUTO_INCREMENT PRIMARY KEY,
      user_id INT NULL,
      phone VARCHAR(30) NOT NULL DEFAULT '254700000000',
...
async function fixOldTables() {
  await addColumnIfMissing("orders", "order_code", "`order_code` VARCHAR(40)");
  await addColumnIfMissing("orders", "customer_id", "`customer_id` INT NULL");
  await addColumnIfMissing("orders", "product_id", "`product_id` INT NULL");
  await addColumnIfMissing("orders", "quantity", "`quantity` INT NOT NULL DEFAULT 1");
  await addColumnIfMissing("orders", "total", "`total` DECIMAL(12,2) NOT NULL DEFAULT 0");
  await addColumnIfMissing("orders", "status", "`status` VARCHAR(40) DEFAULT 'Pending'");
  await addColumnIfMissing("orders", "order_date", "`order_date` DATE NULL");
  await addColumnIfMissing("orders", "created_at", "`created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP");

  await addColumnIfMissing("users", "avatar", "`avatar` TEXT");
  await addColumnIfMissing("users", "is_verified", "`is_verified` BOOLEAN DEFAULT FALSE");

  await addColumnIfMissing("products", "image", "`image` TEXT");
  await addColumnIfMissing("products", "description", "`description` TEXT");
  await addColumnIfMissing("products", "status", "`status` VARCHAR(30) DEFAULT 'active'");

  await addColumnIfMissing("customers", "avatar", "`avatar` TEXT");
  await addColumnIfMissing("customers", "address", "`address` TEXT");

  console.log("Old table migration complete");
}
...
app.get("/", (req, res) => {
  res.json({
    success: true,
    message: "ShopMaster Pro backend is running",
    health: "/api/health",
  });
});
  ...
  try {
    ...
    if (!name || !email || password.length < 6) {
      return res.status(400).json({
        success: false,
        message: "Name, email and 6+ character password required.",
      });
    }
    ...
    await db.query(
      ...
      [result.insertId, verifyToken, "verify", expires]
    );

    const link = `${API_URL}/api/auth/verify-email/${verifyToken}`;

    await sendMail(
      email,
      "Verify your ShopMaster Pro account",
      `
      <h2>Welcome, ${name}</h2>
    ...
    res.status(201).json({
      success: true,
      message: "Account created. Verification link sent to Gmail.",
    });
  } catch (error) {
...
app.post("/api/auth/forgot-password", async (req, res) => {
  try {
    ...
    if (!users.length) {
      return res.json({
        success: true,
        message: "If the Gmail exists, a reset link has been sent.",
      });
    }
    ...
    await db.query(
      "INSERT INTO email_tokens (user_id,token,type,expires_at) VALUES (?,?,?,?)",
      [users[0].id, resetToken, "reset", expires]
    );
    ...
    await sendMail(
      email,
      "Reset your ShopMaster Pro password",
      `
...
app.post("/api/auth/reset-password", async (req, res) => {
  try {
    const resetToken = safeText(req.body.token);
    const password = safeText(req.body.password);
...
app.post("/api/products", auth, async (req, res) => {
  try {
    ...
    if (!name || !category || price < 0 || stock < 0) {
      return res.status(400).json({
        success: false,
        message: "Valid product details required.",
      });
    }
...
app.post("/api/orders", auth, async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      ...
      `,
      [orderCode, customerId || null, productId, quantity, total]
    );
...
app.post("/api/discounts", auth, async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Discount creation failed." });
  }
});
...
app.post("/api/invoices", auth, async (req, res) => {
  try {
    ...
    const invoiceCode = generateCode("INV-");
...
app.post("/api/mpesa/stk-push", async (req, res) => {
  try {
    ...
    if (!phone || phone.length < 12 || amount <= 0) {
      return res.status(400).json({
        success: false,
    ...
    const payload = {
      ...
      AccountReference: "SHOPMASTER",
      TransactionDesc: "ShopMaster payment",
    };
    ...
    await db.query(
      ...
      (phone,amount,merchant_request_id,checkout_request_id,status)
      VALUES (?,?,?,?,?)
      `,
      [
        phone,
